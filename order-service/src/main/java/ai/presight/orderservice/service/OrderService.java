...
public class OrderService {
    ...
    public OrderResponse getOrder(String orderNumber) {
        ...
        var itemResponses = order.getItems().stream().map(i ->OrderItemResponse.builder().skuCode(i.getSkuCode())
        		.quantity(i.getQuantity()).price(i.getPrice()).lineTotal(i.getPrice()* i.getQuantity()).build()).toList();

        return OrderResponse.builder()
                .orderNumber(order.getOrderNumber())
                .message("Fetched successfully")
                .status(order.getStatus())
                .totalAmount(totalAmount).items(itemResponses).createdAt(order.getCreatedAt()).updatedAt(order.getUpdatedAt())
                .build();
    }
